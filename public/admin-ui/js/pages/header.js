...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  const countIsViewed = notification?.data.filter((n) => n.is_viewed === 0).length;
  ...
  const headerNotificationBadge = document.querySelector(".header__notification-badge");
  ...
  if (data && data.success) {
    userElement.textContent = data.user.name || "Người dùng không tên";
    headerUserImage.src = data.user.avatar_url || "/images/default-avatar.jpg";
  } else {
